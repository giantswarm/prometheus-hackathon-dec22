...
func formatMissingUnknownLabelsOrAnnotations(output []string, arrayToParse []string, title string) []string {
	if len(arrayToParse) > 0 {
		output = append(output, fmt.Sprintf("\n      %s: ", title))
		for _, diff := range arrayToParse {
			output = append(output, fmt.Sprintf("%s, ", diff))
		}
